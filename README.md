...
Gmail Threat Analyzer là một dự án phân tích rủi ro email phishing gồm backend FastAPI và Chrome Extension Manifest V3. Backend đọc email Gmail qua IMAP, trích xuất nội dung, URL, attachment, header xác thực và chạy rule-based analyzer để trả về điểm rủi ro cùng bằng chứng. Extension giúp quét email, lưu lịch sử phân tích cục bộ và gắn cảnh báo trực tiếp trên Gmail.
